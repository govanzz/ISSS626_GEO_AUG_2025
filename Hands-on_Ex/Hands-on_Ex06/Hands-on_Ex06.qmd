...
tmap_arrange(TT_HOUSEHOLDS.map, RADIO.map,
             asp=NA, ncol=2)
```
- **Left (Total households):** shows where *more people live*. Darker shades cluster where township population is larger.

- **Right (Number of radios):** largely follows the left map (size effect), **but not perfectly** — some townships with only mid-sized household counts have relatively dark radio counts, while a few large-household townships are lighter for radios.

- **Takeaway:** the radio map is **biased by population size**. You’re mapping **absolute counts** on both, so areas with more households tend to look “high” for radios even if adoption **intensity** isn’t high.
...
There is no one best scaling method to use. One should explore them and select the one that best meet your analysis need.
